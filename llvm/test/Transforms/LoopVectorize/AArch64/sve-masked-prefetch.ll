...
define void @mprefetch_f32(ptr noalias nocapture %a, ptr noalias nocapture readonly %b, i64 %n) {
; CHECK-LABEL: @mprefetch_f32
; CHECK: vector.body:
; CHECK:       %[[LOAD1:.*]] = load <vscale x 4 x float>, ptr
; CHECK-NEXT:  %[[MASK:.*]] = fcmp ogt <vscale x 4 x float> %[[LOAD1]],
; CHECK-NEXT:  %[[TMP9:.*]] = extractelement <vscale x 4 x i64> %[[VEC:.*]], i64 0
; CHECK-NEXT:  %[[GEPA:.*]] = getelementptr float, ptr %a,
; CHECK-NEXT:  call void @llvm.masked.prefetch.p0.nxv4i1(ptr %[[GEPA]], i32 4, i32 0, i32 3, <vscale x 4 x i1> shufflevector (<vscale x 4 x i1> insertelement (<vscale x 4 x i1> poison, i1 true, i64 0), <vscale x 4 x i1> poison, <vscale x 4 x i32> zeroinitializer))
...
if.then:                                          ; preds = %for.body
  ...
  tail call void @llvm.prefetch.p0(ptr nonnull %arrayidx3, i32 0, i32 3, i32 1)
